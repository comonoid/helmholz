...
проведён аудит ЭТОГО ПЛАНА (Г18…Г32). Из порядка работ **пункт 2 выполнен
07-27** — формат файла дерева (magic, версия, два свидетеля представления, полная
целочисленная проверка топологии, CBMC на лестнице конфигураций); доклад в конце
`PLAN_CUT.md`. Следующий — пункт 3 (таблица поверхностей и боковая таблица).
Общий слой пойдёт в `src/cut/`; **`src/octree.*` и `src/cut/` — ОБЩИЕ, а не
