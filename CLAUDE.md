...
## ДИРЕКТИВА ПОЛЬЗОВАТЕЛЯ (11-09): ВОЗМОЖНОСТИ ПАРАЛЛЕЛИЗМА И GPU ДОЛЖНЫ ОСТАВАТЬСЯ

Одноядерность текущих режимов (blk-сбор §884, walk) — ДЕМОНСТРАЦИОННОЕ
СООБЩЕНИЕ, а не архитектурный предел. Требование к любому новому коду:
...
3. Однопоточный режим — дефолт демонстрации; переключатель параллелизма
   не удалять и не ломать (проверка битовости OMP=1 vs N остаётся
   обязательной гейт-процедурой).
