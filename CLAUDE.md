...
**ЧТО ПРИ ЭТОМ НЕ МЕНЯЕТСЯ, ЧТОБЫ НЕ СЛОМАТЬ ЛИШНЕГО.** Отставлена ЛИНИЯ, а не
файлы: `PLAN.md`, `src/` вне `src/transport/`, `src/cut/`, `src/octree.*` и их
тесты остаются в репозитории нетронутыми и продолжают собираться и проходить
`make test`. Ломать, чистить и «прибирать» их нельзя — это не мёртвый код, а
отложенный. Общий слой (`src/cut/`, `src/octree.*`, `PLAN_CUT.md`) остаётся
рабочим и обсуждаемым: он нужен переносу.
