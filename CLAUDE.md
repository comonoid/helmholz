...
**ЧТО ОСТАЛОСЬ В КОРНЕ И ЯВЛЯЕТСЯ ЖИВЫМ:** `PLAN_TRANSPORT.md`, `PLAN_CUT.md`,
`src/transport/`, `src/cut/`, `src/octree.*`, **`src/image.*`** (общий: им
пользуются `render3` и тесты переноса), `tools/render3.c`, `tools/lod3.c`,
`tools/pfmdiff.c`. Граница проверена по зависимостям, а не по названиям:
`src/transport/`, `src/cut/` и `src/octree.*` не включают ни одного волнового
заголовка.
...
формулировкой. СОСТОЯНИЕ: T1 (слой, замкнутая форма), T2 (плоскость, DG1,
рассеяние, печь) и **T5а (марш по лучу, камера, ПЕРВАЯ ТРЁХМЕРНАЯ КАРТИНКА)**
выполнены; доклады в `PLAN_TRANSPORT.md`. Дальше T3, T4, T5б, T6, T7.
**T5 РАСКОЛОТА НАДВОЕ СОЗНАТЕЛЬНО:** её собственный эталон — ламбертова сфера при
удалённом источнике — косвенного света не содержит, поэтому проход 1 (прямой
свет) и проход 3 (сбор по пикселю) проверяемы БЕЗ трёхмерной развёртки, а марш по
лучу нужен проходу 1 при любом порядке вех. **В картинке T5а НЕТ КОСВЕННОГО
СВЕТА, и выдавать её за рендеринг уравнения переноса нельзя** — это два прохода из
трёх. Чего для полной картинки не хватает: трёхмерная развёртка (сетка ГРАНЕЙ над
октодеревом + тетраэдризация разрезанной ячейки + DG1 на многограннике), T3
(сколько ординат), T4 (BRDF, Френель, `n²`, хранимое на поверхности) и разгон DSA
(измерено в T2: 91…125 итераций при альбедо 1).
...
проведён аудит ЭТОГО ПЛАНА (Г18…Г33, затем Г34…Г39 по Р-5а и Г41…Г49 по Р-5б/в).
**08-03 записан Р-7 — ЦЕЛОЧИСЛЕННЫЙ РАЗРЕЗ (Q-формат в раме, вершина из ТРЁХ
ПЛОСКОСТЕЙ вместо интерполяции по ребру) и аудит Г51…Г57 ДО кода. Кода нет.
Г57 говорит прямо: линия покупает применимость CBMC, снятие класса Г31/Г33 и
известную границу ошибки, но НЕ срочна — живой потребитель (перенос) от Г50 не
страдает.**
Из порядка работ **выполнены пункты 2, 4, 3 и 5 целиком (а, б, в) 07-27** —
формат файла дерева (magic, версия, свидетели представления, полная
целочисленная проверка топологии, CBMC), ЯДРО ОТСЕЧЕНИЯ `src/cut/poly3.c` (коробка ×
полуплоскости → топология многогранника + разбиение внешности), ТАБЛИЦЫ
`src/cut/surf.c` (поверхности, фасеты, боковая таблица с ключом-ЯЧЕЙКОЙ, фасетизация
примитива), ФОРМА `src/cut/qef.c` и ЭРМИТОВО ДЕРЕВО `src/cut/dc.c` (таблица рёбер,
QEF-иерархия, починка за O(глубины), обход DC, мост к разрезу). Доклады в конце
`PLAN_CUT.md`. Следующее — пункт 6 (потребители: T4 и хвост №5).
Р-5г/д не пишутся: замер носителей требует сцены T7, которой нет, а
гомогенизация — машинерия ПЕРЕНОСА.
**ДВЕ ВЕЩИ ОСТАЛИСЬ ОТКРЫТЫМИ И ЗАПИСАНЫ, А НЕ ЗАБЫТЫ.** (1) Г50: фальсификатор
2b (побитовость на общей грани у соседей РАЗНЫХ уровней) НЕ ВЫПОЛНЕН — ядро
строит вершину как пересечение РЕБРА с плоскостью, а ребро у крупной и мелкой
ячейки разное, и общая плоскость этого не лечит; полигональная поверхность при
этом БЕЗ ЩЕЛЕЙ (ноль несопряжённых рёбер на перепаде до 8:1), расходятся именно
ОТСЕЧЁННЫЕ ТЕЛА, на `3.6e-13`. Лечение известно (вершина из ТРЁХ ПЛОСКОСТЕЙ по
Крамеру вместо интерполяции), но это правка ядра пункта 4 со своим аудитом.
(2) Г40/Г44: `dmax` у фасетов, выведенных из объёмных данных, НЕ ВЫЧИСЛЯЕТСЯ и
остаётся `UNKNOWN` — невязка QEF есть среднеквадратичное НА ОБРАЗЦАХ, а не
максимум смещения; волновая линия обязана от них отказываться (Г25), переносу они
законны.
...
- LOD ON THE BASIS IS FREE; LOD ON THE GEOMETRY IS AN ACCOUNTING PROBLEM, NOT A
  ...
  (ii) COARSENING IS AGGREGATION — many objects collapsing into ONE surface plus
      ...
      MEASURED 07-27 (Р-5б, tests/test_dc.c), so this is no longer a claim: on a
      sharp crease the hermite fit puts the cell vertex on the crease line to
      EXACTLY 0.0, while the same points with occupancy-style normals (axis
      directions) leave it 1.7e-1 away — the rounding-off is real and it is what
      the normal buys.
