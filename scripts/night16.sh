#!/usr/bin/env bash
# night16.sh — §76: три долга на ГОРОДЕ. Зал уже ответил, но разрыв уровней и
# пустые нижние ступени наблюдались именно на городе, и там же множитель sin бьёт
# сильнее всего (город виден сверху, поверхностей «в лоб» много).
set -u
cd "$(dirname "$0")/.."
