...
static int sb_push(sbctx *B, int32_t ni, const int32_t lo[3], int32_t size, int lvl) {
  ...
  if (size > 1) {
    /* Ш9 (§459). ДО ЭТОГО ЗДЕСЬ БЫЛ ЛОЖНЫЙ НОЛЬ, И ОН БЫЛ НЕ РЕДКОСТЬЮ, А
     * ПРАВИЛОМ: у крупной ячейки сумма не набиралась вовсе, `noct` оставался
     * нулём от memset, а `hz_oct_decode(0)` возвращает не «нормали нет», а
     * вектор (0, 0, −1). Замерено: ровно 100 % крупных ячеек на двух разных
     * сценах (А829), при том что свет читает нормаль ИМЕННО отсюда.
     * Величина берётся из побочного массива дерева — она там уже сложена снизу
     * вверх, и это ВЗВЕШЕННОЕ к центру среднее (А837), а не сумма по 12 рёбрам
     * узла. Путь МЕЛКОЙ ячейки ниже не тронут ни строкой: на этом стоит
     * проверка «при полной глубине срез побитово тот же». */
    hz_dc_nsum(B->t, ni, nsum);
  } else {
    for (int i = 0; i < 12; i++) {
      int a = i / 4, k = i % 4;
      int u = (a + 1) % 3, v = (a + 2) % 3;
