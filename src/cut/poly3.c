 ...
 * (min, max) индексов. Иначе две грани, делящие это ребро, посчитали бы t
 * против 1-t и разошлись бы в последнем бите — щель Г10 внутри ОДНОЙ ячейки. */
static void edge_cross(const hz_poly3 *in, const double *s, int32_t a, int32_t b, double out[3]) {
  int32_t p = a < b ? a : b, q = a < b ? b : a;
