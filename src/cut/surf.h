...
/* §800: потолок вершин куска-полигона. Восемь: выпуклый кусок × 6 коробочных
 * полуплоскостей клипа cut3 даёт ≤ 8 + 6 = 14 вершин при ёмкости клипа 16 —
 * запас не съедается; выше потолка кластеризация просто не сливает. */
#define HZ_FACET_TVMAX 8
...
typedef struct {
  ...
  /* Р-8: ОГРАНИЧЕННЫЙ КУСОК — вершины родителя, ЕДИНИЦЫ кадра. bounded = 0 —
   * куска нет, фасет режет как бесконечная плоскость (прежнее поведение; так
   * приходят аналитическая фасетизация Р-5а и прямые плоскости). Побитовая
   * общность Г3 переносится с плоскости на кусок: вершины полигона — общие
   * данные всех кусков веера, смежные куски делят ребро побитово, соседние
   * коробки клипуют ОДИН кусок.
   * §800: кусок — ВЫПУКЛЫЙ полигон tnv вершин (3 — треугольник Р-8/§794;
   * до HZ_FACET_TVMAX — слитый кластеризацией кусок). */
  int32_t bounded;
  int32_t tnv;
  double tv[HZ_FACET_TVMAX][3];
} hz_facet;
...
/* Плоскость, УЖЕ заданная в единицах кадра. Нужна там, где плоскость родилась из
 * координат дерева, а не из сцены — так приходят фасеты dual contouring (Г46).
 * Мировой вход применил бы кадр ВТОРОЙ раз, и на кубическом кадре u = (1,1,1)
 * это не проявилось бы вовсе: вылезло бы только на анизотропном (Г15,
 * 4 мм × 1 м × 1 м — штатный случай), то есть класс Г21. Обратный перевод «в мир
 * и назад» запрещён: он не тождествен по битам, а водонепроницаемость держится
 * именно на битах. */
int32_t hz_facettab_add_units(hz_facettab *t, const double n[3], double off, int32_t surf,
                              double dmax);

/* Р-8: фасет с ОГРАНИЧЕННЫМ КУСКОМ — треугольник tv в единицах кадра, той же
 * плоскости n·x = off. Потребители без знания о куске работают по-старому
 * (bounded читают только отбор раздачи и клип элемента). */
...
/* §800: фасет с куском-ПОЛИГОНОМ (выпуклый, 3 <= nv <= HZ_FACET_TVMAX, единицы
 * кадра, обход согласован с нормалью). dmax обязан нести ФАКТИЧЕСКОЕ
 * отклонение вершин от плоскости (А1315: ноль здесь — «кусок точен»). */
...
/* --- Р-7а: КВАНТОВАНИЕ ПЛОСКОСТИ В РЕШЁТКУ РАМЫ (PLAN_CUT.md, Г51/Г54) ---
 *
 * Плоскость `n·x <= off` (в ЕДИНИЦАХ рамы, `n` любой длины) переводится в целые
 * той же решётки: `nq` с `HZ_P3_NBITS` разрядами, `offq` в масштабе
 * `2^(HZ_P3_NBITS + HZ_P3_FRAC)`, чтобы предикат `nq·v - offq` считался в целых
 * при вершине `v` в масштабе `2^HZ_P3_FRAC`.
 *
 * ВОЗВРАЩАЕТ ВКЛАД КВАНТОВАНИЯ В `dmax` В МЕТРАХ, и это ТРЕБОВАНИЕ Г51, а не
 * удобство: округление `n` и `off` СДВИГАЕТ и слегка ПОВОРАЧИВАЕТ границу, то
 * есть добавляет своё слагаемое в необъявленную ошибку. Складывать его с уже
 * имеющимся `dmax` обязан вызывающий; там, где `dmax` неизвестен, ворота Г25
 * остаются закрытыми — новое слагаемое их не открывает.
 *
 * `rad_units` — радиус области, на которой смещение оценивается (для ячейки
 * размера `s` единиц это `s·sqrt(3)/2`). Оценка ВЕРХНЯЯ и явная:
 *   d <= (|n - n_q|·rad + |off - off_q|) / |n_q|,  затем × масштаб рамы. */
