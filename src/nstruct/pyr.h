 ...
 * §864/Б1: chmask — бит Existence-карта ДЕТЕЙ (бит cx | cy<<1 | cz<<2):
 * существует ли соответствующий ребёнок на уровне ниже. Заполняется одним
 * подъёмом при построении; марш проверяет существование ДО box_seg —
 * пустой ребёнок обходится бит-тестом, без слэб-теста и двоичного поиска
 * (А1569). Свободный байт паддинга — размер структуры не меняется. */
typedef struct {
  int64_t id;
  uint8_t state;
  uint8_t chmask;
