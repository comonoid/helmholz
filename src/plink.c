...
/* ТОЧНЫЙ ФОРМ-ФАКТОР ТОЧКА–МНОГОУГОЛЬНИК ЖИВЁТ В `src/pff.[ch]`.
 *
 * Вынесен отсюда шагом О55: та же формула нужна стенду фронта, чтобы считать
 * порог по вкладу от ПРЯМОЙ облучённости замкнутой формой (§172, пункт 5;
 * А391). Знак обхода в ней установлен САМОПРОВЕРКОЙ, то есть сценой не
 * ловится, — поэтому второй копии быть не должно. Довод, формула и разбор знака
 * переехали в `pff.h` вместе с кодом; здесь остаются только две обёртки: та,
 * что раскрывает край полигона из местных `(u,v)`, и прежнее имя самопроверки,
 * которым пользуется `tools/prad.c`. */
...
static double lk_ff_loop_uv(const double x[3], const double nrec[3], const double org[3],
  ...
  int m = hz_pff_clip_half(w1, nv, nrec, w2);
  if (m < 3) return 0.0;
  return hz_pff_loop(w2, m, nrec);
}

double hz_links_ff_selftest(double *f_big, double *f_unit, double *ref_unit) {
  return hz_pff_selftest(f_big, f_unit, ref_unit);
}
...
/* ВЕКТОР ОБЛУЧЁННОСТИ `E⃗` — ЧЕТВЁРТОЕ, ПЯТОЕ И ШЕСТОЕ ЧИСЛА ЭЛЕМЕНТА (§85.2, §89).
 *
 * ЗАЧЕМ. Радианс ламбертовой площадки одинаков во все стороны, поэтому по
 * СКАЛЯРНОМУ полю интерполировать нормаль НЕ НАД ЧЕМ (А222): умножать не на что.
 * С вектором облучённости радианс в точке с нормалью `n` есть
 * `ρ/π · (E₀ + E⃗·n)`, и интерполяция нормали начинает работать так, как ей
 * положено — в том числе у ДИФФУЗНЫХ поверхностей.
 *
 * СЧИТАЕТСЯ ОДНИМ ПРОХОДОМ ПОСЛЕ СХОДИМОСТИ, а не каждую итерацию: на решение
 * вектор не влияет, он нужен только съёмке. Поэтому направления берутся из
 * опорных точек на лету, и `hz_link` НЕ РАСТЁТ — иначе 16 байт на связь стали бы
 * 40, а на них держится оценка памяти города в §84.
 *
 * `E₀` здесь — та же сумма `Σ f·B`, что и в решателе, то есть облучённость,
 * делённая на `π`; `E⃗ = Σ f·B·ω̂`, где `ω̂` — единичное направление на излучателя.
 * Спуск по лестнице тот же, что у `acc`: облучённость наследуется потомками. */
void hz_links_evec(const hz_linkset *S, const hz_lod *L, const double *pt, const double *B,
                   double *E0, double *Ev) {
  const int32_t nn = L->nnd;
  for (int32_t k = 0; k < nn; k++) {
    E0[k] = 0.0;
    Ev[3 * k] = Ev[3 * k + 1] = Ev[3 * k + 2] = 0.0;
  }
  for (int64_t l = 0; l < S->n; l++) {
    const hz_link *K = &S->l[l];
    double d[3], r2 = 0.0;
    for (int c = 0; c < 3; c++) {
      d[c] = pt[3 * K->j + c] - pt[3 * K->i + c];
      r2 += d[c] * d[c];
    }
    double w = K->f * B[K->j];
    E0[K->i] += w;
