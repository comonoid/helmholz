...
int32_t hz_lod_cut(const hz_lod *L, const double eye[3], double eps, int nosin, int32_t *out) {
  ...
  for (int32_t k = 0; k < L->np; k++) {
    ...
    for (int lev = 1; lev < L->nlev; lev++) {
      ...
      /* ВТОРОЙ ПРЕДЕЛ: УГЛОВОЙ РАЗМЕР САМОГО ЭЛЕМЕНТА (§136, А300).
       *
       * `dmax < ε·R` ограничивает ошибку ПОЛОЖЕНИЯ и не ограничивает РАЗМЕР
       * вовсе. На воксельной сцене это разрушительно: `dmax = 0` ТОЧНО на любой
       * плоскости, сколь угодно большой, поэтому критерий пропускает узлы в
       * сотни метров. Замерено: в кадре города видно `123` элемента из
       * `152 618` (§120), оболочки каскада не расслаиваются (§134), а LOD даёт
       * ноль по фрагментам (§131) — все три упираются сюда.
       *
       * Новый предел — тот же ε и то же `R`, никакого нового порога:
       * `rad/R < ε_разм`, где `ε_разм = eps · szmul`. При `szmul = 0` предел
       * снят и поведение прежнее, то есть правка обратно совместима. */
      int ok = (nd->dmax * sn < eps * R);
      if (ok && L->szmul > 0.0) ok = (nd->rad < eps * L->szmul * R);
      if (ok)
        sel = id;
      else
        break;
