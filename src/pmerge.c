...
#pragma omp atomic write
  ...
  /* ТАКТИКА ПОЛОС ПО УГЛУ (§59). При `cfg->bands` ключом сортировки становится не
   * ошибка, а УГОЛ между нормалями пары, разложенный по полосам: внутри полосы
   * порядок прежний (по ошибке), а полосы идут по возрастанию угла. Тогда все
   * компланарные слияния завершаются транзитивно прежде, чем тронуто хоть одно
   * наклонное, и между полосами происходит переоценка. */
  if (cfg->bands) {
    for (int64_t i = 0; i < pn; i++) {
      ...
      /* Полосы: `0`, до `1°`, `2`, `5`, `10`, `20`, `45`, дальше. Числа не
       * подобраны под результат — это двоично-десятичная шкала, и её выбор
       * записан как параметр замера, а не как найденная константа. */
      int bk = 7;
      const double edge[7] = {1e-9, 1.0, 2.0, 5.0, 10.0, 20.0, 45.0};
      for (int q = 0; q < 7; q++)
        if (th <= edge[q]) {
          bk = q;
          break;
