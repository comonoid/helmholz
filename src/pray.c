  ...
  for (int pass = 0; pass < 2; pass++) {
    ...
    for (int32_t k = 0; k < ps->np; k++) {
      ...
      for (int a = 0; a < 3; a++) {
        ...
        /* ЗАЖИМАЮТСЯ ОБА КОНЦА И С ОБЕИХ СТОРОН. Первая редакция зажимала `i0`
         * только снизу, а `i1` только сверху, и на этом падала: полигон,
         * упирающийся в ВЕРХНЮЮ границу габарита, даёт `t0 = nc` ровно, то есть
         * `i0 = nc`; `i1` после зажима становится `nc−1`, и следующая строка
         * `i1 < i0 ⇒ i1 = i0` возвращает `nc` обратно. Дальше `start[c+1]++`
         * пишет за конец массива. На зале не срабатывало (там ни один участок
         * не вырождается в точку на верхней грани габарита), на ГОРОДЕ — сразу:
         * ASan, `heap-buffer-overflow` в `hz_pray_build`. Это не ошибка
         * упрощения края, а ошибка сетки лучей, найденная масштабом. */
        if (i0 < 0) i0 = 0;
        if (i0 > g->nc[a] - 1) i0 = g->nc[a] - 1;
        if (i1 < 0) i1 = 0;
