...
void hz_psweep_solve(hz_ptrans *t, hz_pstats *st) {
  ...
  for (int32_t k = 0; k < t->np; k++) {
    ...
    double c[3];
    /* §127: снять дефект квадратуры по нормали — до подгонки, чтобы поправка не
     * размазалась по трём коэффициентам по-разному. */
    if (t->qn != NULL && t->qn[k] > 0.0) {
      b[0] /= t->qn[k];
      b[1] /= t->qn[k];
      b[2] /= t->qn[k];
    }
    ...
    /* ИЗМЕНЕНИЕ МЕРЯЕТСЯ ПО ЭНЕРГИИ, А НЕ ПО КОЭФФИЦИЕНТАМ, и это не вкус.
     * Осколок площадью меньше пикселя получает в накопитель ЦЕЛЫЙ фрагмент
     * (`h²·L·w`), а делится на свою точную площадь `M[0]`, которая много меньше
     * `h²`. Коэффициент выходит огромным — измерено: при `mom[0] = 7.5e−6`
     * такой один полигон забивал `Σ|c|` настолько, что `dE` падало до `7.6e−9`
     * на ПЕРВОМ отскоке, и развёртка «сходилась», не перенеся ничего.
     * `∫E dA` у осколка мал при любом `c`, поэтому энергетическая норма к этому
     * невосприимчива по построению. */
    double ie_old = e[0] * M[0] + e[1] * M[1] + e[2] * M[2];
    for (int a = 0; a < 3; a++)
      e[a] = c[a];
    double ie_new = e[0] * M[0] + e[1] * M[1] + e[2] * M[2];
    sd += fabs(ie_new - ie_old);
    ss += fabs(ie_new);
    double ie = ie_new;
    phi_in += ie;
