...
/* МОМЕНТЫ ПЕРЕСЧИТЫВАЮТСЯ ПО КРАЮ — ЗАМКНУТОЙ ФОРМОЙ, БЕЗ КВАДРАТУРЫ (А127).
 *
 * ЗАЧЕМ. Моменты `∫1 ∫u ∫v ∫u² ∫uv ∫v²` считаются при сборке ПО ТРЕУГОЛЬНИКАМ, а
 * подгонка вершин двигает КРАЙ. После неё элемент перестаёт быть согласован сам с
 * собой: контур один, интегралы от другого. Для метрики это незаметно (луч
 * смотрит на плоскость и край), а для СБОРКИ УРАВНЕНИЙ это прямая ошибка — там
 * матричные элементы берутся именно из моментов.
 *
 * ФОРМУЛЫ ТОЧНЫЕ (Грин по контуру), а не квадратура: для многоугольника с
 * вершинами `(u_i, v_i)` против часовой стрелки
 *   `A   = ½ Σ c_i`,                    где `c_i = u_i v_{i+1} − u_{i+1} v_i`,
 *   `∫u  = ⅙ Σ (u_i + u_{i+1}) c_i`,
 *   `∫u² = 1/12 Σ (u_i² + u_i u_{i+1} + u_{i+1}²) c_i`,
 *   `∫uv = 1/24 Σ (2u_i v_i + u_i v_{i+1} + u_{i+1} v_i + 2u_{i+1} v_{i+1}) c_i`.
 * Дыры обходятся по часовой (`polygon.h`), поэтому их вклад вычитается САМ, без
 * особого случая. */
static void vf_moments(hz_polyset *ps, int32_t k) {
  hz_poly *P = &ps->p[k];
  if (P->nloop <= 0) return;
  ...
  for (int32_t l = P->l0; l < P->l0 + P->nloop; l++) {
    ...
    for (int32_t i = 0; i < n; i++) {
      ...
      m[1] += (A[0] + B[0]) * c / 6.0;
      m[2] += (A[1] + B[1]) * c / 6.0;
      m[3] += (A[0] * A[0] + A[0] * B[0] + B[0] * B[0]) * c / 12.0;
      m[4] += (2.0 * A[0] * A[1] + A[0] * B[1] + B[0] * A[1] + 2.0 * B[0] * B[1]) * c / 24.0;
      m[5] += (A[1] * A[1] + A[1] * B[1] + B[1] * B[1]) * c / 12.0;
    }
  }
  for (int i = 0; i < 6; i++)
    P->mom[i] = m[i];
  /* ПЛОЩАДЬ ЭЛЕМЕНТА И ПЛОЩАДЬ ПРЕДСТАВЛЯЕМОЙ ПОВЕРХНОСТИ — ДВЕ РАЗНЫЕ ВЕЛИЧИНЫ,
   * И `area` НЕ ПЕРЕЗАПИСЫВАЕТСЯ (А128). `mom[0]` есть площадь САМОГО элемента
   * (он плоский, поэтому его двумерная площадь и есть трёхмерная); `P->area`
   * пришла суммой площадей ИСХОДНЫХ треугольников, которые наклонены к плоскости
   * и потому больше. Их отношение и есть та самая потеря площади при уплощении
   * (§38), а на слипание она же — подпись Ш3. Замер поймал разницу до `1.0`
   * относительной величины: у части элементов контур охватывает почти ноль при
   * ненулевой представляемой поверхности. Перезаписать `area` значило бы стереть
   * эту улику и подсунуть в сборку уравнений нулевой элемент. */
}
...
int hz_poly_vfit(hz_polyset *ps, double dmove, hz_vfitstat *st) {
   ...
   * Заодно пересчитываются МОМЕНТЫ (А127): элемент обязан быть согласован сам с
   * собой, иначе сборка уравнений возьмёт интегралы от другой геометрии. */
  for (int32_t k = 0; k < ps->np; k++) {
    hz_poly *P = &ps->p[k];
    ...
    vf_moments(ps, k);
    if (a0 > 0.0) {
      /* Отношение «площадь элемента / площадь представляемой поверхности»:
       * единица — уплощение ничего не потеряло, ноль — контур схлопнулся. */
      double rel = fabs(P->mom[0] - a0) / a0;
      if (rel > st->dmax_area) st->dmax_area = rel;
    }
