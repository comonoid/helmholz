...
static int cut_cell(const hz_facettab *ft, const hz_cutmap *cm, const int32_t *tri_of,
                    ...
                    int32_t fkey[][4], int maxp, int *nfb_out) {
  ...
  *nfb_out += p.nfb; /* Г53: отказ Крамера обязан считаться; на этой сцене их быть не должно */
  int np = 0;
...
static void t_2b(hz_dctree *t) {
  ...
  for (int rot = 0; rot < 2; rot++) {
    ...
    int pairs = 0, exact = 1, cmp = 0, nfbsum = 0;
    ...
    for (int i = 0; i < nb; i++) {
      ...
      for (int j = 0; j < nb; j++) {
        ...
        int na = cut_cell(&ft, &cm, g_b.tri_of, &bx[i], xf, pa, ka, fa, 64, &nfbsum);
        int nbp = cut_cell(&ft, &cm, g_b.tri_of, &bx[j], xf, pb, kb, fb, 64, &nfbsum);
    ...
    printf("  [2b, %s] пар соседей разных уровней %d, точек от ОДНИХ И ТЕХ ЖЕ плоскостей %d, "
           "побитово=%d, расхождение между ячейками %.1e, fallback Крамера %d\n",
           rot ? "НК плоскость на ячейку" : "общая плоскость", pairs, cmp, exact, maxd, nfbsum);
    ...
    if (rot == 0) {
      ...
      /* ПРИЁМКА Р-7б (Г56, фальсификатор 2b): точка на общей грани определена
       * ТРЕМЯ плоскостями, и ядро считает её из них НАПРЯМУЮ (Крамер по
       * канонической тройке, plane_canon + plane_less), а не интерполяцией по
       * ребру — ребро у крупной и мелкой ячейки РАЗНОЕ (Г50). Совпадение
       * ОБЯЗАНО быть побитовым: нулевое расхождение, не допуск. */
      check(exact, "2b: точки от ОДНИХ плоскостей на шве уровней совпадают ПОБИТОВО");
      check(!(maxd > 0.0), "2b: расхождение между ячейками РОВНО ноль");
      /* вершина обязана лежать на своей тройке плоскостей: сравнение с прямым
       * 2x2-решением — sanity хорошо обусловленных троек, а не побитовость
       * (формула 2x2 другая, биты её не обязаны повторять). */
      check(maxa < 1e-9 && maxb < 1e-9,
            "вершины лежат на тройке плоскостей (сверка с прямым решением)");
      /* Г53: fallback Крамера (det == 0 на дублях плоскостей веера, вершина
       * в концу ребра) ЗАСЧИТАН в nfbsum и напечатан выше — приёмкой служит
       * сама побитовость 2b, а не ноль отказов: отказ обязан быть ВИДИМЫМ,
       * а не невозможным. */
    } else {
      /* НЕГАТИВНЫЙ КОНТРОЛЬ (предсказан провал, Г52): та же геометрия, но
       * плоскость треугольника КАЖДАЯ ячейка считает по своему порядку вершин
       * — тройки плоскостей РАЗНЫЕ в битах, и вершины ОБЯЗАНЫ разойтись.
       * Если совпадут — сравнение ничего не мерит. */
      check(cmp > 0, "НК: есть что сравнивать");
      check(!exact && maxd > 0.0, "НК: чужая плоскость ОБЯЗАНА разойтись в битах");
    }
    hz_facettab_free(&ft);
    hz_cutmap_free(&cm);
  }
