...
static void t_bitwise(void) {
  ...
  /* Р-7б, Г52: вершина теперь функция ЗНАЧЕНИЙ плоскостей, а не рёбер.
   * Одна и та же стена стоит у соседей РАЗНЫМИ битами (у A — грань hi стены,
   * (-n,-off)-форма; у B — грань lo, (-n,-off)... каноническая форма
   * plane_canon обязана свести их; без неё 2a падает на ulp — найдено этим
   * тестом, а не угадано. Согласие ПЕРЕВЁРНУТЫХ фасетов в настоящем
   * дополнении проверяют test_surf (Г3) и test_facet — там сравниваются
   * соименные куски, здесь при перевороте полупространства многогранник
   * другой и попарное сравнение не имеет смысла.
   *
   * Прежний НЕГАТИВНЫЙ контроль Г23 «в локальных координатах биты расходятся»
   * при Р-7б УМЕР и заменён: сдвиг кадра при осевых стенах поглощается в off
   * ТОЙ ЖЕ арифметикой, что и в самом Крамере, и биты сходятся —
   * нечувствительность к сдвигу кадра стала СВОЙСТВОМ нового выражения
   * (усиление Г39), проверять обратное больше нечестно. Место негативного
   * контроля занял контроль битовой ЧУВСТВИТЕЛЬНОСТИ ниже (off на один ulp). */
  /* НЕГАТИВНЫЙ КОНТРОЛЬ (предсказан провал): off, сдвинутый на ОДИН ulp.
   * Геометрия меняется на ~1e-16 относительной, биты вершины ОБЯЗАНЫ
   * разойтись — иначе match_on_wall ничего не проверяет. */
  int exact_u = 1, pairs_u = 0;
  {
    hz_poly3 bu;
    hz_hspace hu;
    memcpy(hu.n, n, sizeof hu.n);
    hu.off = nextafter(off, 1e300);
    int32_t uid = 0;
    check(hz_poly3_cut(&bu, blo, bhi, &hu, &uid, 1) == HZ_P3_OK, "ulp-вариант построен");
    pairs_u = match_on_wall(&a, &bu, 4.0, &exact_u);
    check(pairs_u >= 2, "ulp-вариант даёт те же точки геометрически");
    check(!exact_u, "негативный контроль: ulp-off ОБЯЗАН разойтись в битах");
  }

  /* Локальные координаты: печатается, не навязывается. При Р-7б биты здесь
   * сходятся (свойство, см. выше); для трёх КОСЫХ граней расхождение не
   * исключено, и фальсификатором это объявлять нельзя. */
  ...
  printf("  [Г23] пар на стене %d, побитово=%d; ulp-off разошёлся (НК, %d пар);\n"
         "        локальные координаты: побитово=%d (при Р-7б сходятся — свойство)\n",
         pairs, exact, pairs_u, exact_loc);
}
...
int main(void) {
  ...
  printf("  Г23 off на один ulp -> биты ОБЯЗАНЫ разойтись (Р-7б: сдвиг кадра биты больше НЕ "
         "меняет)\n");
