...
static void hall_light(arealight *AL, const opyr *P, const frame *fr, const double lo[3],
  ...
  /* ПАНЕЛЬ ЗАПОДЛИЦО С ПОТОЛКОМ, А НЕ ВИСЯЩАЯ ПОД НИМ (правка 08-11 по картинке):
   * отступ 0.10 м делал из площадки ПАРЯЩИЙ светильник, и N×N его проб рисовали
   * на потолке сетку пятен — артефакт выборки, а не свет. */
